...
def main():
        ...
        for env in ["pendulum", "cartpole"]:
            for policy in ["noise", "sparse"]:
                ...
                if len(good) < 3:
                    ...
                    continue
                slope, intercept, r = fit_line(good["true_gl"].values, good["measured_gl"].values)
                ...
                for tag in TAG_ORDER:
                    ...
                    if len(sub) == 0:
                        continue
                    s, i, rr = fit_line(sub["true_gl"].values, sub["measured_gl"].values)
    ...
    for arch, df in arch_dfs.items():
        ...
        pend["measured_gl"] = pd.to_numeric(pend["measured_gl"], errors="coerce")
        ...
        for mtype in (["trained", "random"] if has_type else ["trained"]):
            ...
            good = sub[(sub["r2"] >= args.min_r2) & np.isfinite(sub["measured_gl"])]
            n_valid = len(good)
            n_total = len(sub)
            if n_valid < 3:
                rows[f"{arch}/{mtype}"] = {
                ...
                continue
            slope, _, r = fit_line(good["true_gl"].values, good["measured_gl"].values)
            rows[f"{arch}/{mtype}"] = {
