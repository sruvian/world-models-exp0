...
def collect_trajectories(env, num_trajectories: int, episode_time: int, policy_seed:int, save: bool = True):
    ...
    for trajectory in range(num_trajectories):
        ...
        for time_step in range(episode_time):
            ...
            env.step(action)
            trajectory_states.append(env.get_state())
    ...
    if save:
        ...
        save_file = os.path.join(datasets_dir,
                                f"v0_{env.__class__.__name__}_N{num_trajectories}_T{episode_time}_env{env.env_seed}_pol{policy_seed}_dt{env.dt:.3f}_damp{env.damping:.3f}.npz")
