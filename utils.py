...
def parse_model(path: Path) -> dict:
    ...
    result = {
        ...
        "N": 0,
        "beta": None,
        "model_name": "WorldModelVAE" if "WorldModelVAE" in name else "WorldModel",
    }
    ...
    # components is parts after model name (parts[0]) and model type (parts[1])
    components = parts[2:]
    ...
    if "combined" in components:
        ...
        for part in components:
            if part.startswith("k"):
                try: result["k"] = int(part[1:])
                except ValueError: pass
            elif part.startswith("latent"):
                try: result["latent"] = int(part[6:])
                except ValueError: pass
            elif part.startswith("steps"):
                try: result["steps"] = int(part[5:])
                except ValueError: pass
            elif part.startswith("beta"):
                try: result["beta"] = float(part[4:])
                except ValueError: pass
    else:
